...
public class ServerIntegrationTests(ServerIntegrationFixture Server) : IClassFixture<ServerIntegrationFixture>
{
    ...
    public async Task HostctlClient_CanListScreensForMediaSource()
    {
        ...
        await Task.WhenAll(Enumerable.Range(0, screensTarget)
            ...
            .Select(_ => client.SendEvent(BuildScreenCreateRequest(mediaSourceCreate?.Data?.Id), cts4.Token)));
        ...
        await Task.Run(async () =>
        {
            ...
            {
                await Task.Yield();
            } while (screens.Count < screensTarget);
        }, cts5.Token);
        ...
        Assert.NotNull(mediaSourceCreate?.Data?.Id);
        var cts6 = new CancellationTokenSource(TimeSpan.FromSeconds(5));
        await client.SendEvent(BuildMediaSourceListScreensRequest(mediaSourceCreate.Data.Id), cts6.Token);

        // Wait a few seconds for responses
        var cts7 = new CancellationTokenSource(TimeSpan.FromSeconds(5));
        await Task.Run(async () =>
        {
            do
            {
                await Task.Yield();
            } while (responseScreens.Count < screens.Count);
        }, cts7.Token);
